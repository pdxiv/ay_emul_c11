...
static void pattern_interpreter(psm_file* f, psm_channel* chan) {
    ...
    if (op <= 0x5F) {
      ...
      if (((uint8_t)chan->orn_tick & 0x40) != 0 && chan->orn >= 33) {
        ...
        if (chan->env_type >= 0xB1) {
          ay_chip_set_ay_register_fast(&f->ay.chip, 13,
...
static void change_registers(psm_file* f, psm_channel* chan,
                              uint8_t* temp_mixer) {
  uint8_t b, b1, b2;
  uint16_t w, wo, ws;
...
/* Players.pas:11974-12281, PSM_Get_Registers (minus CheckLoopAndStop -
 ...
 * advance. */
static void psm_get_registers(psm_file* f) {
  uint8_t temp_mixer;

  f->global_tick_counter++;

  ay_chip_set_ay_register_fast(&f->ay.chip, 8, 0);
  ay_chip_set_ay_register_fast(&f->ay.chip, 9, 0);
  ay_chip_set_ay_register_fast(&f->ay.chip, 10, 0);
  ...
  if (f->delay_counter == 0) {
    ...
    if (f->chan_a.note_skip_counter == 0) pattern_interpreter(f, &f->chan_a);
    f->delay_counter = f->delay;
  }

  temp_mixer = 0;
  change_registers(f, &f->chan_a, &temp_mixer);
  change_registers(f, &f->chan_b, &temp_mixer);
  change_registers(f, &f->chan_c, &temp_mixer);
